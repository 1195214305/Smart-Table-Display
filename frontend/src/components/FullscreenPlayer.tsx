...
import { Play, Pause, RotateCcw, Settings, X, Edit3, Check, Save, Palette } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { ExcelData } from '@/hooks/useExcelReader'
import { PlaybackSettings } from './ScrollingTable'
...
const FullscreenPlayer: React.FC<FullscreenPlayerProps> = ({
  ...
  const [titleFontSize, setTitleFontSize] = useState(initialSettings.titleFontSize)
  const [backgroundColor, setBackgroundColor] = useState(initialSettings.backgroundColor || '#1a1a1a')
  const [tableFrameColor, setTableFrameColor] = useState(initialSettings.tableFrameColor || '#ff6b35')
  ...
  useEffect(() => {
    ...
    setRowHeight(initialSettings.rowHeight)
    setTitleFontSize(initialSettings.titleFontSize)
    setBackgroundColor(initialSettings.backgroundColor || '#1a1a1a')
    setTableFrameColor(initialSettings.tableFrameColor || '#ff6b35')
  ...
  useEffect(() => {
    ...
    const handleMouseMove = () => {
      ...
      controlsTimeoutRef.current = setTimeout(() => {
        if (isPlaying && !isEditingTitle && !showSettings) {
  ...
  }, [isVisible, isPlaying, onClose, isEditingTitle, showSettings])
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor }}>
      {/* 控制栏 */}
      <div
        className={`absolute top-0 left-0 right-0 bg-black/80 backdrop-blur-sm text-white p-4 z-20 transition-opacity duration-300 ${
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm opacity-75">空格键播放/暂停，ESC退出全屏</span>
            <Button
              onClick={onClose}
              size="sm"
              variant="secondary"
        ...
        {showSettings && (
          <div className="space-y-4 mt-4 p-4 bg-black/50 rounded-lg max-h-96 overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full"
                />
                <span className="text-sm opacity-75">{speed}px/frame</span>
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full"
                />
                <span className="text-sm opacity-75">{fontSize}px</span>
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full"
                />
                <span className="text-sm opacity-75">{rowHeight}px</span>
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full"
                />
                <span className="text-sm opacity-75">{titleFontSize}px</span>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    />
                    <span className="text-sm opacity-75">{backgroundColor}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {backgroundPresets.map((color) => (
                      <button
                        key={color}
                        className={`w-8 h-8 rounded border-2 ${backgroundColor === color ? 'border-accent-cyan' : 'border-gray-400'}`}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <input
                      ...
                      className="w-12 h-8 rounded border cursor-pointer"
                    />
                    <span className="text-sm opacity-75">{tableFrameColor}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {frameColorPresets.map((color) => (
                      <button
                        key={color}
                        className={`w-8 h-8 rounded border-2 ${tableFrameColor === color ? 'border-accent-cyan' : 'border-gray-400'}`}
                        style={{ backgroundColor: color }}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <input
                      ...
                      className="w-12 h-8 rounded border cursor-pointer"
                    />
                    <span className="text-sm opacity-75">{textColor}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {textColorPresets.map((color) => (
                      <button
                        key={color}
                        className={`w-8 h-8 rounded border-2 ${textColor === color ? 'border-accent-cyan' : 'border-gray-400'}`}
                        style={{ backgroundColor: color }}
      ...
      <div
        ...
        style={{
          paddingTop: showSettings ? '420px' : showControls ? '100px' : '20px'
        }}
      >
        {/* 标题区域 */}
        <div
          className="px-8 py-3 text-center border-b-4 shadow-lg"
          style={{
            backgroundColor,
            borderBottomColor: tableFrameColor,
            boxShadow: `0 4px 6px -1px ${tableFrameColor}40`
          }}
        >
          {isEditingTitle ? (
            <div className="flex items-center justify-center gap-2">
              <Input
                ...
                className="text-center max-w-6xl border-2"
                style={{
            ...
            <div className="flex items-center justify-center gap-2">
              <h1
                className="font-bold cursor-pointer hover:bg-white/10 px-4 py-2 rounded-lg transition-colors min-h-[3rem] flex items-center text-center leading-tight"
                style={{
                  fontSize: `${titleFontSize}px`,
                  color: textColor,
            ...
            <thead className="sticky top-0 z-10">
              <tr>
                {data.headers.map((header, index) => (
                  <th
                    key={index}
                    className="px-8 py-6 text-left font-bold border-r-2 last:border-r-0"
                    style={{
                      height: `${rowHeight}px`,
                      borderColor: '#ffffff',
              ...
              {repeatedRows.map((row, rowIndex) => (
                ...
                >
                  {data.headers.map((_, colIndex) => (
                    <td
                      key={colIndex}
                      className="px-8 py-4 border-r border-gray-700 last:border-r-0"
                      style={{
                        height: `${rowHeight}px`,
